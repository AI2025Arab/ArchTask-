...
func (d *dbTaskSearcher) Search(opts *taskSearchOptions) (tasks []*Task, totalCount int64, err error) {
	...
	// ArchTask Addition - Filter by architectural phase if provided.
	var archPhaseCond builder.Cond
	if opts.archPhase != "" {
		archPhaseCond = builder.Eq{"tasks.arch_phase": opts.archPhase}
	}

	cond := builder.And(builder.Or(projectIDCond, favoritesCond), where, filterCond, archPhaseCond)
